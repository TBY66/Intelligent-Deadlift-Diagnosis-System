"""
appv1.3.py – Intelligent Deadlift Diagnosis System  v1.3
Based on appv1.2.  One change: feedback is generated asynchronously by a
FeedbackWorker thread that calls DiagnosisEngine.generate_feedback_with_llm()
(GGUF model → rule-based fallback).  The result is cached in HistoryEntry so
re-selecting an already-analysed video does not re-run inference.
"""
...
from PyQt5.QtGui import (
    ...
    QFontDatabase, QPainter, QPen,
)

# ── load pipeline via importlib so the dotted filename works ──────────────────
_spec = importlib.util.spec_from_file_location(
    "pipelinev1_3",
    os.path.join(os.path.dirname(__file__), "pipelinev1.3.py"),
)
_mod = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_mod)
DiagnosisEngine = _mod.DiagnosisEngine
...
def _resolve_fonts():
    db   = QFontDatabase()
    fams = db.families()
    title = ("Barlow Condensed" if "Barlow Condensed" in fams
             else "SF Pro Display" if "SF Pro Display" in fams
             else "Helvetica Neue")
    body  = ("Barlow" if "Barlow" in fams
              else "SF Pro Text" if "SF Pro Text" in fams
              else "Helvetica Neue")
    return title, body
...
class HistoryEntry:
    def __init__(self, original_name, video_path):
        ...
        self.results       = []
        self.analyzed      = False
        self.nose_trace   = None   # (sample_frames, nose_y) | None
        self.analyze_mode = "standard"  # "standard" | "fast"
...
class AnalysisWorker(QThread):
    ...
    def __init__(self, video_path, mode: str = "standard"):
        super().__init__()
        self.video_path = video_path
        self.mode       = mode
    ...
    def run(self):
        try:
            ...
            self.finished.emit(reps, results, ENGINE._last_nose_trace)
...
class FeedbackWorker(QThread):
    ...
    def __init__(self, results, style: str):
        super().__init__()
    ...
    def run(self):
        try:
            text = DiagnosisEngine.generate_feedback_styled(self.results,
                                                            self.style)
...
class UploadArea(QFrame):
    ...
    def _build(self):
        ...
        icon      = QLabel()
        icon_path = os.path.join(os.path.dirname(__file__), "materials",
                                 "upload_icon.png")
        if os.path.exists(icon_path):
            pix = QPixmap(icon_path).scaled(68, 68, Qt.KeepAspectRatio,
            ...
            icon.setStyleSheet(
                f"color:{SURFACE2}; background:transparent; border:none;"
            )
        ...
        title.setStyleSheet(
            f"color:{TEXT_P}; background:transparent; border:none; "
...
class VideoPlayer(QWidget):
    ...
    def _seek_released(self):
        self._user_seeking = False
        self._cur = max(0, min(int(self.slider.value() * self._total / 1000),
                               self._total - 1))
        self._show_frame(self._cur)
...
# ═══════════════════════════════════════════════════════════════════════════════
# Nose Trajectory chart (unchanged from v1.2)
# ═══════════════════════════════════════════════════════════════════════════════
class NoseChart(QWidget):
    ...
    def paintEvent(self, event):
        ...
        pen = QPen(QColor(SURFACE2));  pen.setWidthF(0.5)
        p.setPen(pen)
        for frac in (0.25, 0.5, 0.75):
            gy = int(PT + frac * ph)
            p.drawLine(PL, gy, PL + pw, gy)

        pen = QPen(QColor(AMBER));  pen.setStyle(Qt.DashLine);  pen.setWidthF(1.0)
        p.setPen(pen)
        for bf in self._rep_bounds:
            x = int(_cx(bf));  p.drawLine(x, PT, x, PT + ph)

        pen = QPen(QColor(ACCENT));  pen.setWidthF(1.5);  pen.setCapStyle(Qt.RoundCap)
        p.setPen(pen)
        px_prev = py_prev = None
        for fi, ny in zip(self._sample_frames, self._nose_y):
            if np.isnan(ny):
                px_prev = py_prev = None;  continue
            cx_, cy_ = int(_cx(fi)), int(_cy(ny))
            if px_prev is not None:
                p.drawLine(px_prev, py_prev, cx_, cy_)
            px_prev, py_prev = cx_, cy_

        pen = QPen(QColor(TEXT_S));  pen.setWidthF(1.0)
        p.setPen(pen)
        phx = int(_cx(self._cur_frame))
        p.drawLine(phx, PT, phx, PT + ph)

        p.setPen(QColor(TEXT_M));  p.setFont(F_SMALL)
        p.drawText(QRect(0, PT, PL - 3, 14), Qt.AlignRight | Qt.AlignVCenter, "↑")
        p.drawText(QRect(0, PT + ph - 14, PL - 3, 14),
...
class AnalysisPanel(QFrame):
    ...
    def _build(self):
        ...
        self._summary.setStyleSheet(
            f"color:{TEXT_S}; background:transparent; border:none;"
        )
...
class FeedbackPanel(QFrame):
    style_changed = pyqtSignal(str)   # "professional" | "friendly"

    _BTN_ON  = (f"QPushButton{{background:{ACCENT};color:#000;font-weight:bold;"
                f"border:none;border-radius:5px;padding:2px 10px;}}")
    _BTN_OFF = (f"QPushButton{{background:{SURFACE};color:{TEXT_S};"
                f"border:1px solid {BORDER_A};border-radius:5px;padding:2px 10px;}}"
                f"QPushButton:hover{{background:{SURFACE2};color:{TEXT_P};}}")
    ...
    def _build(self):
        ...
        self._btn_fri = QPushButton("Friendly")
        for btn in (self._btn_pro, self._btn_fri):
            btn.setFont(F_BADGE)
            btn.setCursor(Qt.PointingHandCursor)
        self._btn_pro.clicked.connect(lambda: self._switch("professional"))
        self._btn_fri.clicked.connect(lambda: self._switch("friendly"))
        hdr_row.addWidget(self._btn_pro)
        hdr_row.addWidget(self._btn_fri)
    ...
    def _refresh_toggle(self):
        self._btn_pro.setStyleSheet(
            self._BTN_ON if self._style == "professional" else self._BTN_OFF
        )
        self._btn_fri.setStyleSheet(
            self._BTN_ON if self._style == "friendly" else self._BTN_OFF
        )
...
class HistoryItemWidget(QWidget):
    ...
    analyze_requested = pyqtSignal(str, str)   # (entry_id, mode)
    ...
    def _menu(self):
        ...
        if not self._analyzed:
            sub = m.addMenu("Analyze")
            sub.setStyleSheet(MENU_SS)
            sub.addAction("Standard  (yolov11s)",
                          lambda: self.analyze_requested.emit(self.entry_id, "standard"))
            sub.addAction("Fast  (yolov11n)",
                          lambda: self.analyze_requested.emit(self.entry_id, "fast"))
            m.addSeparator()
...
class HistoryPanel(QFrame):
    ...
    analyze_requested = pyqtSignal(object, str)   # (entry, mode)
    ...
    def _build(self):
        ...
        btns = QWidget();  btns.setStyleSheet("background:transparent;")
        bl   = QVBoxLayout(btns)
        bl.setContentsMargins(10, 4, 10, 4);  bl.setSpacing(5)
        ...
        for b in (self.btn_all, self.btn_del):
            b.setFont(F_SMALL);  b.setCursor(Qt.PointingHandCursor)
        self.btn_all.setStyleSheet(BTN_SEC)
        self.btn_del.setStyleSheet(BTN_DANGER)
        self.btn_all.clicked.connect(self._ask_analyze_all)
        self.btn_del.clicked.connect(self._delete_all)
        row.addWidget(self.btn_all);  row.addWidget(self.btn_del)

        bl.addWidget(self.btn_upload);  bl.addLayout(row)
        lay.addWidget(btns)
    ...
    def add_entry(self, entry: HistoryEntry):
        ...
        w.analyze_requested.connect(self._analyze_one)   # (eid, mode)
    ...
    def refresh_entry(self, entry: HistoryEntry):
        if entry.id in self._widgets:
            self._widgets[entry.id].update_name(entry.display_name,
                                                entry.analyzed)
    ...
    def _analyze_one(self, eid: str, mode: str):
        e = self._entries.get(eid)
        if e:
            self.analyze_requested.emit(e, mode)

    def _ask_analyze_all(self):
        m = QMenu(self)
        m.setStyleSheet(MENU_SS)
        m.addAction("Standard  (yolov11s)", lambda: self._analyze_all("standard"))
        m.addAction("Fast  (yolov11n)",     lambda: self._analyze_all("fast"))
        m.exec_(QCursor.pos())

    def _analyze_all(self, mode: str):
...
class ProgressOverlay(QFrame):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setStyleSheet(
            f"background:rgba(2,6,23,215); border:1px solid {BORDER_A}; "
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._fb_worker:        FeedbackWorker | None = None
        self._fb_entry:         HistoryEntry | None   = None
        self._fb_style_pending: str                   = "professional"
    ...
    def _build(self):
        ...
        for col in (DANGER, AMBER, ACCENT):
            ...
            tb_lay.addWidget(d);  tb_lay.addSpacing(5)
        ...
        tb_lay.addWidget(title_lbl);  tb_lay.addStretch()
        vlay.addWidget(tbar)
        ...
        self._left.addWidget(self._upload)
        self._left.addWidget(self._player)
        ...
        self._hist.analyze_requested.connect(self._queue_analysis)   # (entry, mode)
        ...
        self._feedback.style_changed.connect(self._on_style_changed)
    ...
    def _on_files(self, paths):
        ...
        if last:
            ...
            self._mid.hide()
            self._status.set(
                f"Added {len(paths)} video(s).  Click Analyze to start.", TEXT_S
            )

    # ── analysis queue ─────────────────────────────────────────────────────────
    def _queue_analysis(self, entry: HistoryEntry, mode: str = "standard"):
    ...
    def _start(self, entry: HistoryEntry):
        ...
        self._worker = AnalysisWorker(entry.video_path, mode=entry.analyze_mode)
    ...
    def _on_done(self, reps, results, nose_trace):
        ...
        if done:
            ...
            if self._current_entry and self._current_entry.id == done.id:
                self._display(done)
    ...
    def _display(self, entry: HistoryEntry):
        ...
        if entry.nose_trace is not None:
            ...
            self._nose_chart.clear()
        self._show_feedback_for_style(entry)

    # ── feedback style helpers ─────────────────────────────────────────────────
    def _on_style_changed(self, style: str):
        if self._current_entry and self._current_entry.analyzed:
            self._show_feedback_for_style(self._current_entry)

    def _show_feedback_for_style(self, entry: HistoryEntry):
        style  = self._feedback.current_style()
        cached = entry.feedback.get(style)
        if cached is not None:
            self._feedback.show_feedback(cached)
        else:
            self._feedback.show_feedback("Generating feedback…")
            self._start_fb_worker(entry, style)

    def _start_fb_worker(self, entry: HistoryEntry, style: str):
        if self._fb_worker and self._fb_worker.isRunning():
            try:
                self._fb_worker.finished.disconnect()
                self._fb_worker.error.disconnect()
            except RuntimeError:
                pass
        self._fb_entry         = entry
        self._fb_style_pending = style
        self._fb_worker        = FeedbackWorker(entry.results, style)
        self._fb_worker.finished.connect(self._on_fb_done)
        self._fb_worker.error.connect(self._on_fb_error)
        self._fb_worker.start()

    def _on_fb_done(self, text: str):
        fb_entry    = self._fb_entry
        fb_style    = self._fb_style_pending
        self._fb_entry = None
        if fb_entry:
            fb_entry.feedback[fb_style] = text
            if (self._current_entry
                    and self._current_entry.id == fb_entry.id
                    and self._feedback.current_style() == fb_style):
                self._feedback.show_feedback(text)

    def _on_fb_error(self, msg: str):
        fb_entry    = self._fb_entry
        fb_style    = self._fb_style_pending
        self._fb_entry = None
        if (fb_entry
                ...
                and self._current_entry.id == fb_entry.id
                and self._feedback.current_style() == fb_style):
            self._feedback.show_feedback("Feedback unavailable.")
