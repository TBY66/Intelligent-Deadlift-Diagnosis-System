"""
pipelinev1.3.py – Intelligent Deadlift Diagnosis System processing backend.

Based on pipelinev1.2 with one addition:
  generate_feedback_with_llm(results) – tries the fine-tuned GGUF model first,
  falls back to the rule-based generate_feedback(results) if unavailable.

The GGUF model was trained on a hybrid SFT dataset where each example maps a
structured diagnosis (case_type / rep labels) to a stylistically varied coaching
reply.  The text it produces is still grounded in the same four fault categories
(c / h / k / r); only tone and phrasing differ across samples.
"""
...
_GGUF_PATH = os.path.join(BASE_DIR, "feedback_model",
                          "smollm2_deadlift_hybrid_q4_k_m.gguf")
...
def _get_llm():
    ...
    try:
        ...
        # Use Metal on Apple Silicon, CUDA elsewhere; 0 = CPU-only fallback
        use_gpu = torch.cuda.is_available() or torch.backends.mps.is_available()
        n_gpu   = -1 if use_gpu else 0
        _llm = Llama(
            model_path=_GGUF_PATH,
            n_ctx=512,           # prompt + reply < 300 tokens; smaller ctx = faster init
...
# ── MoveNet Lightning (lightweight pose for rep detection) ────────────────────
# Download: https://storage.googleapis.com/download.tensorflow.org/models/tflite/movenet/movenet_singlepose_lightning.tflite
# Save as:  yolo_models/movenet_lightning.tflite
_MOVENET_PATH  = os.path.join(BASE_DIR, "yolo_models", "movenet_lightning.tflite")
_movenet_interp = None
_movenet_tried  = False


def _get_movenet():
    global _movenet_interp, _movenet_tried
    if _movenet_tried:
        return _movenet_interp
    _movenet_tried = True
    if not os.path.isfile(_MOVENET_PATH):
        return None
    try:
        try:
            import tflite_runtime.interpreter as _tflite
            _movenet_interp = _tflite.Interpreter(model_path=_MOVENET_PATH)
        except ImportError:
            import tensorflow as _tf
            _movenet_interp = _tf.lite.Interpreter(model_path=_MOVENET_PATH)
        _movenet_interp.allocate_tensors()
    except Exception:
        _movenet_interp = None
    return _movenet_interp


def _movenet_pose(interp, frame_bgr):
    """
    Run MoveNet Lightning on one BGR frame.
    Returns (nose_y_px: float, wrist_avg_xy: np.ndarray [x,y]) or (None, None).
    Keypoint order matches YOLO_KEYS (COCO 17-point).
    """
    H, W = frame_bgr.shape[:2]
    rgb  = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
    img  = cv2.resize(rgb, (192, 192))[np.newaxis]   # uint8 (1,192,192,3)
    ...
    if dtype == np.float32:
        ...
        img = (img.astype(np.float32) - 128.0).astype(np.int8)
    # np.uint8 needs no conversion

    interp.set_tensor(in_det["index"], img)
    interp.invoke()
    kps = interp.get_tensor(out_det["index"])[0, 0]  # (17,3): [y_norm, x_norm, conf]

    if float(kps[0, 2]) < 0.2:
        return None, None

    nose_y = float(kps[0, 0]) * H

    # indices 9=left_wrist, 10=right_wrist  (same as COCO / YOLO_KEYS)
    lw = np.array([kps[9,  1] * W, kps[9,  0] * H])   # [x, y]
    rw = np.array([kps[10, 1] * W, kps[10, 0] * H])
    return nose_y, (lw + rw) / 2.0


# ── default cues (fallback when GGUF unavailable) ────────────────────────────
_DEFAULT_CUES: dict[str, dict[str, str]] = {
    "professional": {
        "c": "Excellent execution — maintain this consistent technique.",
        "h": ("Your hips rose too early. "
              "Focus on driving through your legs and raise your chest simultaneously."),
        "k": ("Your knees track too far forward with an overly upright torso. "
              "Prioritise hip hinge initiation."),
        "r": "Brace your core and maintain lumbar extension throughout the lift.",
    },
    "friendly": {
        "c": "Great job — keep it up!",
        "h": ("Hips shot up a bit early! Try pushing through both legs evenly "
              "so your chest and hips rise together."),
        "k": ("Knees are travelling too far forward. "
              "Think about sitting back and hinging at the hips more!"),
        "r": "Remember to brace your belly tight and lock in that lower back!",
    },
}
_cue_cache: dict[tuple, str] = {}


def _get_cue(fault: str, style: str) -> str:
    """Return a styled cue for one fault label.  Tries GGUF; falls back to defaults."""
    key = (fault, style)
    ...
    if llm is not None:
        try:
            ...
            pass
    cue = _DEFAULT_CUES.get(style, _DEFAULT_CUES["professional"]).get(
        fault,
        _DEFAULT_CUES["professional"].get(fault, ""),
    )
    _cue_cache[key] = cue
    return cue
...
# ════════════════════════════════════════════════════════════════════════════
# Neural-network definitions  (must match train_model.py exactly)
# ════════════════════════════════════════════════════════════════════════════
...
class TF1(nn.Module):
    def __init__(self, input_dim=30, embed_dim=64, depth=3, num_heads=4):
        ...
        self.fc_out    = nn.Sequential(
            ...
            nn.Dropout(0.4), nn.Linear(64, 66)
        )
...
class TF2(nn.Module):
    def __init__(self, f1_dim=30, f3_dim=66, embed_dim=128, depth=3,
                 num_heads=4, dropout=0.3):
        super().__init__()
        def _proj(d):
            return nn.Sequential(
                nn.Linear(d, embed_dim), nn.LayerNorm(embed_dim),
        ...
        self.f1_to_f3_attn      = nn.MultiheadAttention(embed_dim, num_heads,
                                                         dropout=dropout,
                                                         batch_first=True)
...
def _back_curve_kv(mask_np, shoulder, hip, nose=None, eps=1e-6):
    ...
    orient = None
    if nose is not None:
        cv_ = np.array([cx, cy]) - mid
        rel  = float(np.dot(cv_, up)) * side
...
class DiagnosisEngine:
    """
    Instantiate once; call process_video() per video.
    progress_cb(message: str, percent: int)
    """

    def __init__(self):
        self._loaded          = False
        self._last_nose_trace = None   # (sample_frames, nose_y) | None
        self._active_pose     = None   # set by process_video based on mode
    ...
    def load_models(self, progress_cb=None):
        ...
        _p(f"Loading YOLO pose models… (device: {DEVICE})", 5)
        self.pose   = YOLO(os.path.join(BASE_DIR, "yolo_models", "yolov11s-pose.pt"))
        self.pose_n = YOLO(os.path.join(BASE_DIR, "yolo_models", "yolov11n-pose.pt"))

        _p("Loading YOLO segmentation models…", 12)
        self.seg    = YOLO(os.path.join(BASE_DIR, "yolo_models", "yolov11s-seg.pt"))
        self.seg_n  = YOLO(os.path.join(BASE_DIR, "yolo_models", "yolov11n-seg.pt"))

        _p("Loading YOLO equipment model…", 19)
        self.equip  = YOLO(os.path.join(BASE_DIR, "yolo_models",
        ...
        self.tf2_thr = ckpt2["threshold"]
        self.tf2.eval()

        _p("Loading XGBoost…", 40)
        self.xgb = XGBClassifier()
        self.xgb.load_model(
            os.path.join(BASE_DIR, "inference_models", "xgb_rounded_fold5.json")
        )
        with open(os.path.join(BASE_DIR, "inference_models",
                               "xgb_rounded_fold5_thr.txt")) as fh:
            self.xgb_thr = float(fh.read().strip())

        self._loaded = True
        _p("All models loaded.", 45)

    # ── public API ───────────────────────────────────────────────────────────
    def process_video(self, video_path, progress_cb=None, mode: str = "standard"):
        """
        mode: "standard" (yolov11s) | "fast" (yolov11n)

        Returns
        -------
        reps    : list[(start_frame, end_frame)]
        results : list[np.ndarray([c, h, k, r])]
        """
        if not self._loaded:
            self.load_models(progress_cb)

        if mode == "fast":
            self._active_pose = self.pose_n
            self._active_seg  = self.seg_n
        else:
            self._active_pose = self.pose
            self._active_seg  = self.seg
    ...
    def _scan_rep_signals(self, video_path, imgsz):
        ...
        n             = len(sample_frames)
        equip_cutoff  = int(total * 0.15)  # equip YOLO only for first 15% of video

        nose_y   = np.full(n, np.nan, dtype=float)
        equip_y  = np.full(n, np.nan, dtype=float)
        wrist_xy = [None] * n
        ...
        for si, fi in enumerate(sample_frames):
            ...
            # ── pose: MoveNet if available, YOLO pose as fallback ────────────
            if movenet is not None:
                ny, wxy = _movenet_pose(movenet, frame)
                if ny is not None:
                    nose_y[si]   = ny
                    wrist_xy[si] = wxy
            else:
                pr = self._active_pose(rgb, verbose=False, device=DEVICE, imgsz=imgsz)[0]
            ...
            # ── equip YOLO: first 30% of video only ──────────────────────────
            if fi <= equip_cutoff:
    ...
    def _build_rep_ranges(self, sample_frames, nose_y, equip_y, fps, polarity):
        sample_rate = (fps / max(1, sample_frames[1] - sample_frames[0])
                       if len(sample_frames) > 1 else fps)
        ...
        if ev.sum() >= 5:
            ...
            ey_s   = (savgol_filter(ey, ey_win, 3)
                      if len(ey) >= ey_win else ey.copy())
        ...
        ny_win = _odd_window(len(nose_y), cap=REP_NOSE_SMOOTH_WIN_CAP,
                             floor=min(11, REP_NOSE_SMOOTH_WIN_CAP))
        ny_s         = (savgol_filter(nose_y, ny_win, 3)
                        if len(nose_y) >= ny_win else nose_y.copy())
        ...
        loose_delta  = 0.03 * signal_range

        if polarity == "max":
            raw_peaks, _ = find_peaks(ny_s, distance=min_distance,
                                      prominence=min_prom)
            candidates = [int(i) for i in raw_peaks
                          if ny_s[i] > (mean_ny - loose_delta)]
        else:
            raw_peaks, _ = find_peaks(-ny_s, distance=min_distance,
                                      prominence=min_prom)
            candidates = [int(i) for i in raw_peaks
        ...
        filtered  = [v for v in candidates
                     if v >= max(0, equip_start_idx - grace_idx)]
        if len(filtered) < 2:
            filtered = candidates
    ...
    def _detect_reps(self, video_path):
        ...
        total, fps, sample_frames, nose_y, equip_y = \
            self._scan_rep_signals(video_path, imgsz=320)
        if total < 2:
            return []
        ...
        if valid.sum() < 5:
            total, fps, sample_frames, nose_y, equip_y = \
                self._scan_rep_signals(video_path, imgsz=640)
            if (~np.isnan(nose_y)).sum() < 5:
                return []

        nose_y = _interp_nans(nose_y)
        self._last_nose_trace = (np.array(sample_frames, dtype=int),
                                 nose_y.copy())

        reps = self._build_rep_ranges(sample_frames, nose_y, equip_y, fps,
                                      polarity="max")
        if not reps:
            reps = self._build_rep_ranges(sample_frames, nose_y, equip_y, fps,
                                          polarity="min")
    ...
    def _extract_features(self, video_path, start_f, end_f):
        """
        Returns (20, 33) float32 array.
        Batches all 20 frames through pose in one call, then batches only the
        frames that need segmentation in a second call — rather than 20 separate
        forward passes for each model.
        """
        ...
        for fi in idxs:
            ...
            if ret and frame is not None:
                try:
                    frames_bgr.append(np.ascontiguousarray(frame))
                except Exception:
        ...
        # video dimensions (same for every frame)
        hw = next((f.shape[:2] for f in frames_bgr if f is not None), (480, 640))
        H, W = hw

        frames_rgb = [
            cv2.cvtColor(f, cv2.COLOR_BGR2RGB) if f is not None else None
            for f in frames_bgr
        ]

        # ── Phase 2: batch pose inference ────────────────────────────────────
        valid_idx  = [i for i, f in enumerate(frames_rgb) if f is not None]
        pose_batch = [frames_rgb[i] for i in valid_idx]

        pose_map: dict[int, object] = {}   # frame_slot → YOLO Result
        if pose_batch:
            for slot, result in zip(valid_idx,
                                    self._active_pose(pose_batch, verbose=False,
                                                      device=DEVICE)):
                pose_map[slot] = result

        # ── Phase 3: extract kp info + f1 features ───────────────────────────
        def _parse_pose(pr):
            """Return (kp, kp_conf, box) or (None, None, None)."""
            if (pr.keypoints is None
                    or pr.keypoints.xy is None
                    or pr.keypoints.xy.shape[0] == 0):
            ...
            if pr.boxes is not None and len(pr.boxes) > 1:
                ...
                pi = 0
            kp_      = kpts_all[pi]
            conf_    = (pr.keypoints.conf[pi].cpu().numpy()
                        if pr.keypoints.conf is not None else np.ones(17))
            box_     = (pr.boxes.xyxy[pi].cpu().numpy()
                        if pr.boxes is not None and len(pr.boxes) > pi else None)
            return kp_, conf_, box_
        ...
        def _f1_from_kp(kp):
            ...
            if torso > 1e-6:
                kn /= torso
            jkp    = {nm: kn[KP_IDX[nm]] for nm in J_TARGET}
            coords = np.concatenate([jkp[j] for j in J_TARGET])
        ...
        kp_info  = []   # (kp, kp_conf, box) or (None,None,None) per slot
        f1_rows  = []
        for i in range(NUM_FRAMES):
            if i not in pose_map:
                kp_info.append((None, None, None))
                f1_rows.append(np.full(30, np.nan))
                continue
            kp, kp_conf, box = _parse_pose(pose_map[i])
            kp_info.append((kp, kp_conf, box))
            f1_rows.append(_f1_from_kp(kp) if kp is not None else np.full(30, np.nan))

        # ── Phase 4: batch seg — only frames with a valid shoulder ───────────
        seg_slots = []
        for i, (kp, kp_conf, box) in enumerate(kp_info):
            if kp is None or kp_conf is None or frames_rgb[i] is None:
                continue
            if (kp_conf[6] > kp_conf[5] and kp_conf[12] > kp_conf[11]) or \
               (kp_conf[5] >= kp_conf[6] and kp_conf[11] >= kp_conf[12]):
                seg_slots.append(i)

        seg_map: dict[int, object] = {}
        if seg_slots:
            seg_batch = [frames_rgb[i] for i in seg_slots]
            for slot, result in zip(seg_slots,
                                    self._active_seg(seg_batch, verbose=False,
                                                     device=DEVICE)):
                seg_map[slot] = result

        # ── Phase 5: back features ────────────────────────────────────────────
        fback_rows = []
        for i in range(NUM_FRAMES):
            k_val, o_val, v_val = np.nan, 0, np.nan
            kp, kp_conf, box = kp_info[i]
            ...
            if i in seg_map and box is not None:
                sr = seg_map[i]
                if sr.masks is not None:
                    masks_np = sr.masks.data.cpu().numpy()
                    ...
                    if best_mi >= 0:
                        m_res  = cv2.resize(masks_np[best_mi], (W, H),
        ...
        # ── Phase 6: assemble ─────────────────────────────────────────────────
        f1 = np.array(f1_rows,    dtype=float)
        fb = np.array(fback_rows, dtype=float)
        ...
        return np.concatenate([f1, fb], axis=1).astype(np.float32)

    # ── inference ────────────────────────────────────────────────────────────
    def _classify(self, f5):
        """Returns np.array([correct, hip, knee, rounded], dtype=int)."""
        f1_30 = torch.tensor(f5[:, :30]).unsqueeze(0).to(DEVICE)

        with torch.no_grad():
            f3, _, _ = self.tf1(f1_30)
    ...
    # ── rule-based feedback (identical to v1.2) ───────────────────────────────
    @staticmethod
    def generate_feedback(results):
        """
        results : list of np.array([c, h, k, r])
        Returns a multi-line feedback string.
        """
        if not results:
        ...
        n    = len(results)
        errs = [
            [key for key in ("h", "k", "r")
             if results[i][{"h": 1, "k": 2, "r": 3}[key]]]
            for i in range(n)
        ]
        ...
        part_errs = {
            p: [key for key in ("h", "k", "r")
                if part_res[p][{"h": 1, "k": 2, "r": 3}[key]]]
            for p in parts
        }
    ...
    # ── styled feedback (GGUF cues + v1.2 structure) ─────────────────────────
    @classmethod
    def generate_feedback_styled(cls, results, style: str = "professional") -> str:
        """
        Same structure as generate_feedback() but each fault cue is supplied by
        _get_cue(fault, style).  GGUF is tried first; _DEFAULT_CUES is the fallback.
        ...
        def _etxt(keys):
            return "  ".join(_get_cue(k, style) for k in keys)
        ...
        if n == 1:
            return _get_cue("c", style) if ok[0] else _etxt(errs[0])

        if n == 2:
            if ok[0] and ok[1]:
                return _get_cue("c", style)
            if ok[0]:
                return f"You did rep 1 well, but the other rep — {_etxt(errs[1])}"
            if ok[1]:
                return f"You did rep 2 well, but the other rep — {_etxt(errs[0])}"
