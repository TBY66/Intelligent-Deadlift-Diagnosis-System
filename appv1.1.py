"""
appv2.py – Intelligent Deadlift Diagnosis System  v2.0
...
# ── Skill-derived palette (OLED Dark Mode + Financial Dashboard) ──────────────
# Source: ui-ux-pro-max → colors.csv "Financial Dashboard"
BG        = "#020617"   # OLED-optimised bg (near-black)
CARD      = "#0F172A"   # Primary card surface
SURFACE   = "#1E293B"   # Secondary surface
SURFACE2  = "#334155"   # Hover / tertiary
BORDER    = "#1E293B"   # Subtle border
BORDER_A  = "#334155"   # Active / focus border

# CTA green (confirmed by skill: Financial + IoT Dashboard CTA)
ACCENT    = "#22C55E"
ACC_DIM   = "#16A34A"
ACC_BG    = "#052E16"
ACC_GLOW  = "#4ADE80"

# Amber highlight (skill CTA recommendation for analytics dashboards)
AMBER     = "#F59E0B"
AMBER_BG  = "#1C1000"

TEXT_P    = "#F8FAFC"   # 7:1+ on #020617 – WCAG AAA
TEXT_S    = "#94A3B8"
TEXT_M    = "#475569"
...
# ── Font loading (Barlow Condensed / Barlow – Sports/Fitness pairing) ─────────
def _resolve_fonts():
    db = QFontDatabase()
    fams = db.families()
...
# Resolved after QApplication exists (see main())
...
def card_ss(r=12, border=None):
    b = border or BORDER
    return (
        f"background:{CARD}; border:1px solid {b}; border-radius:{r}px;"
    )
...
def _glow(widget, color=ACCENT, radius=18):
    """Attach a subtle drop-shadow glow to a widget (skill: minimal glow)."""
...
# ═══════════════════════════════════════════════════════════════════════════════
# Rep result card  (skill: green = positive indicator, amber = caution)
# ═══════════════════════════════════════════════════════════════════════════════
_FAULT_META = [
    ("Correct",        ACCENT,  ACC_BG),    # green
    ("Hip first",      AMBER,   AMBER_BG),  # amber – from skill CTA highlight
    ("Knee dominant",  INFO,    "#060F2A"),  # blue
    ("Rounded back",   DANGER,  "#1A0505"),  # red
]
...
class RepCard(QFrame):
    def __init__(self, rep_idx, result_vec, fps, rep_range, parent=None):
        ...
        if rep_range and fps:
            ...
            # amber for duration – skill amber = highlight data point
...
# ═══════════════════════════════════════════════════════════════════════════════
# Progress overlay  (skill: loading-states + minimal glow)
# ═══════════════════════════════════════════════════════════════════════════════
...
class MainWindow(QMainWindow):
    ...
    def _on_done(self, reps, results):
...
def main():
    ...
    # resolve fonts after QApplication exists
